...
class HuffmanCoder{
    ...
    getMappings(node, path){
        if(typeof(node[1])==="string"){  //leaf nodes [freq,char] typeof(char)==string not true for internal nodes
            this.mappings[node[1]] = path; //node[1] is the char hence mappings stored for only leaf nodes(data is present here only)
                                            //char=>binary code
            return;
        }
    ...
    encode(data){
       ...
       //insert {freq,char } in min heap/priority queue
               for(const key in mp){
            this.heap.insert([-mp[key], key]);   //leaf nodes->[freq,ch]
        }
        //construct huffman tree
        while(this.heap.size() > 1){
            const node1 = this.heap.extractMax(); //leaf nodes->[freq,ch]
            const node2 = this.heap.extractMax();

            const node = [node1[0]+node2[0],[node1,node2]];  //tree internal nodes transformed-> [freq,[left,right]];
                                                            //leaf nodes remains same->[freq,ch]
            this.heap.insert(node);
        }
        const huffman_encoder = this.heap.extractMax(); //tree root node

        //get mapppings by traversing  tree char->encoded string e.g a-01 b-10
        this.mappings = {};
        this.getMappings(huffman_encoder, "");
        
        //get binary string from character string eg "aba"->"011001"
        let binary_string = "";
        for(let i=0;i<data.length;i++) {
